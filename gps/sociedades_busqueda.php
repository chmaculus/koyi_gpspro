...
<center>
<titulo>Busqueda sociedades</titulo>
<?php
$limite=20;
$desde=$_POST["desde"];
$hasta=$_POST["hasta"];
...
$query='select * from sociedades where sociedad like "%'.$_POST["busqueda"].'%"';
...
echo '<br><font1>Mostrando Resultados desde: '.($desde+1).' Hasta: '.$hasta.' de: '.$total_rows.'</font1><br><br>';
...
while($row=mysql_fetch_array($result)){
	...
	echo '<td><A HREF="sociedades_ingreso.php?id_sociedades='.$row["id"].'"><button>Modificar</button></A></td>';
	echo '<td><A HREF="sociedades_eliminar.php?id_sociedades='.$row["id"].'"><button>Eliminar</button></A></td>';
	echo "</tr>".chr(13);
}
echo "</table>";
